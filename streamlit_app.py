...
slider = alt.binding_range(min=2009, max=2017, step=1)
select_year = alt.selection_single(name="Year", fields=['Year'], bind=slider, init={'Year': 2010})
...
chart5_animated = chart.add_selection(
    select_year
).transform_filter(
    select_year
)
